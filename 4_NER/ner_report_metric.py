...
def report_metric(opts, logger):
    ...
    for example in data:
        ...
        # ===========直接在这里算=============
        is_count = False
        for item in strict_predict_list:
            if not (item[1] in example['seq'].lower() or item[1].capitalize() in example['seq'] or item[1].upper() in example['seq']):
            # if not (item[1] in example['seq'] or item[1]in example['seq'].capitalize()  or item[1] in example['seq'].upper()):
                is_count = True
                num_hallucination += 1
                print("预测出的不在文本中的单词为：" + item[1])
            if item[0] not in e_types_list:
                is_count = True
                num_hallucination += 1
                print("预测出不在预定集合内的类型：" + item[0])

        if is_count:
            num_hallucination_sample +=1
            print("这是第"+str(num_hallucination_sample)+"个出现幻觉的原句子：" + example["seq"])
    ...
    print("num_invalid："+str(num_invalid))
    print("num_hallucination："+str(num_hallucination))
    print("num_hallucination_sample："+str(num_hallucination_sample))
