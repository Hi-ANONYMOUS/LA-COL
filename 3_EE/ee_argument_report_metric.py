...
def argument_report_metric(opts, logger, file_name=None, dump_to_file=False):
    ...
    for example in data:
        ...
        for event in example["event"]:
            ...
            # 判断是否出现了虚幻
            # 遍历列表中的元素
            if example['text'] == "The surge in rigs suggest that U.S. oil drillers were getting comfortable with crude prices at around $ 40 per barrel .":
                print("这里")
            for sublist in rels_list:
                ...
                for index, item in enumerate(sublist):
                    ...
                    #             break
                    if index == 1 or item in example['text'] or item.capitalize() in example['text'] or item.upper() in example['text']:
                        continue
                    else:
                        if not is_count:
                            is_count = True
