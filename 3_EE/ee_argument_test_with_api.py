...
def get_prompt_list():
    ...
    # 3
    prompt = 'According to the given text and an event it expresses, identify all arguments of this event from the given text, and select a role from the given candidate roles for each argument. The argument is an entity that appears in the given text and participates in this event. Since the arguments in the given text may come from multiple events, please identify only the arguments of the given event.\nAnswer in the format ["argument", "role"] without any explanation. If no argument is involved, then just answer "[]". Importantly, please complete this task well based on the text and the dependency tree  information of the text. Please combine the given text and its constituency information to accomplish the task.'
    prompt_list.append(prompt)
...
def ee_role_args_get_prompt(opts, text_str, evt_type, evt_roles, prompt_list, prompt_icl_list, prompt_cot_list, resource):
    ...
    if opts.ICL:
        prompt = prompt_list[opts.best_prompt] + "\n" + prompt_icl_list[
            opts.prompt - 1] + '\nGiven text:\n"{}"\nEvent type:\n"{}"\nCandidate roles:\n{}\nAnswer:\n'.format(
            input_text, evt_type, json.dumps(evt_roles))
    elif opts.COT:
        prompt = prompt_list[opts.best_prompt] + "\n" + prompt_cot_list[
            opts.prompt - 1] + '\nGiven text:\n"{}"\nEvent type:\n"{}"\nCandidate roles:\n{}\nAnswer:\n'.format(
            input_text, evt_type, json.dumps(evt_roles))
        ...
        prompt = prompt_list[
            ...
            input_text, resource, evt_type, json.dumps(evt_roles))
    return prompt
    ...
    with open(opts.result_file, 'a', encoding='utf-8') as fw:
        ...
        while i < len(selected_idx):
            ...
            for evt in example["event"]:
                ...
                prompt = ee_role_args_get_prompt(opts, text_str, evt_type, evt_roles, prompt_list, prompt_icl_list,
                                                 prompt_cot_list, resource)
