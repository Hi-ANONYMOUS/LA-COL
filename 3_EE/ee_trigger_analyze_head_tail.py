...
from ee_trigger_report_metric import trigger_report_metric_head_tail
cur_path = os.path.abspath("..")
sys.path.append(cur_path)
from utils import Logger


# 将jsonlines文件转变为json文件

def jsonlines_to_json(jsonlines_file, json_file):
    with open(jsonlines_file, 'r', encoding='utf-8') as f:
        jsonlines_data = [json.loads(line.strip()) for line in f]

    with open(json_file, 'w', encoding='utf-8') as f:
        json.dump(jsonlines_data, f, indent=4)
    ...
    with open(in_file_name, 'r', encoding='utf-8') as f,\
        ...
        for example in data:  
            for event in example["event"]:
                for sub_event in event['mentions']:
                    rel_type = sub_event["subtype"]
...
if __name__ == "__main__":
    # 将jsonlines转变为json
    # jsonlines_file = "D:\\Work 4\\Codes\llama-test\\3_EE\data\ee\casie\\train_processed.jsonlines"
    # json_file = "D:\\Work 4\\Codes\llama-test\\3_EE\data\ee\casie\\train.json"
    # jsonlines_to_json(jsonlines_file, json_file)
    ...
    trigger_report_metric_head_tail(opts, logger)