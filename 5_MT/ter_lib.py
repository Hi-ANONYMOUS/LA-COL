...
from y_utils import bot_run_deepseek_vanilla, bot_run_llama_vanilla
...
def generate_ans(model, module, prompt, parser):
    ...
    # =========该用deepseek==========
    bot = OpenAI(api_key="", base_url="https://api.deepseek.com")
    # response = bot_run_deepseek_vanilla(bot, prompt)
    response = bot_run_llama_vanilla(bot, prompt)
    print(response)
    if response=="":
        ans_mt = "未翻译出来"
    else:
        ans_dict = parser.parse(response)
