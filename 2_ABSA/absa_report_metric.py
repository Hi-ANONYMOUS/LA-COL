        ...
        if "AE" == key:
            ...
            for item in res_list:
                if "'s" in item:
                    ...
                    item = item.replace("'s", " 's")
                if item in example["raw_words"]:
                    continue
                else:
                    num_hallucination += 1
                    print("预测出的单词："+item)
                    print("原句子："+example["raw_words"])
                    break
            if not flag:
                num_invalid += 1
        ...
        if "Triplet" == key:
            ...
            for sublist in res_list:
                ...
                for index, item in enumerate(sublist):
                    ...
                    #         if sub_item.capitalize() in example["raw_words"] or sub_item.upper() in example["raw_words"]:
                    #             is_count = True
                    #             num_hallucination += 1
                    #             print("预测出的单词：" + sub_item)
                    #             print("原句子：" + example["raw_words"])
                    #             break
                    if index == 1 or item in example["raw_words"] or item.capitalize() in example["raw_words"] or item.upper() in example["raw_words"]:
                        continue
                    else:
                        if not is_count:
                            is_count = True
...
def get_metric(opts, logger):
    ...
    if opts.irrelevant:
        ...
        if "wang" in opts.dataset:
            ...
            # task_list = ["OE", "ALSC_wang", "AESC_wang"]  #
        if "fan" in opts.dataset:
            task_list = ["AOE"]
