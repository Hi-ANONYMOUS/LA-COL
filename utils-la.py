...
# def bot_create(**para):
#     return openai.Completion.create(**para).choices[0]

def bot_run(bot, prompt, text_str, model="gpt-3.5-turbo-1106"):
    # ####################################=======================Chat=======================####################################
    messages = [
        {"role": "system", "content": "You are a famous expert on Event Extraction, especially expertise in Event Argument Extraction. "},
    ...
    # ####################################=======================Instruct=======================####################################
    # response_linguistics = bot_create(model=model, prompt=prompt, temperature = 0.0)
    # response_linguistics_text = response_linguistics["text"].strip().strip("\n")
    # return response_task_text, prompt
