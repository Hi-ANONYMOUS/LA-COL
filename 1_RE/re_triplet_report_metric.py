...
def triplet_report_metric(opts, logger, file_name=None, dump_to_file=False):
    ...
    for example in data:
        ...
        # 判断是否出现了虚幻
        # 遍历列表中的元素
        if example['seq'] == "The surge in rigs suggest that U.S. oil drillers were getting comfortable with crude prices at around $ 40 per barrel .":
            print("这里")
        is_count = False
        for sublist in rels_list:
            # 将字典的值转换为列表
            sublist = list(sublist.values())
            if is_count:
                break
            for index, item in enumerate(sublist):
                ...
                #         if not (sub_item in example["text"] or sub_item.capitalize() in example["text"] or sub_item.upper() in example["text"]):
                #             is_count = True
                #             num_hallucination += 1
                #             print("预测出的单词：" + sub_item)
                #             print("原句子：" + example["text"])
                #             break
                if index == 2 or item in example['seq'] or item.capitalize() in example['seq'] or item.upper() in \
                        example['seq']:
                    continue
                else:
                    if not is_count:
