...
def get_prompt_list(r_types, e_types_dict):
    ...
    if len(e_types_dict) != 0:
        ...
        prompt = 'According to the given text, please find out all relations expressed by each specified entity pair mentioned in the text from the predefined list of relations.\nPredefined relation list:\n{}\nThe given entity pairs are listed in the form ("subject", "object").\nAnswer in the format \'("subject", "object"): ["relation_1", "relation_2", ...]\' without any explanation. Please combine the given text and its constituency information to accomplish the task.'.format(
            json.dumps(r_types))
        prompt_list.append(prompt)

        prompt = 'What relations are expressed by each given subject-object entity pair in the given text? Return one or more relations from the list {}. The given entity pairs are listed in the form ("subject", "object").\nAnswer in the format \'("subject", "object"): ["relation_1", "relation_2", ...]\' without any explanation.'.format(
            json.dumps(r_types))
        prompt_list.append(prompt)
...
def re_rc_get_prompt(opts, example, resource, r_types, e_types_dict, prompt_list, prompt_icl_list, prompt_cot_list):
    ...
    if opts.ICL:
        ...
        # linguistic information                                                                                               pairs_str)
        prompt = prompt_list[
                     opts.prompt - 1] + '\nGiven text:\n"{}"\nThe constituency information of the given text:\n"{}"\nEntity pairs:\n{}\nAnswer:\n'.format(
            input_text, resource, pairs_str)
...
def re_rc_main(opts, bot, logger):
    ...
    with open(opts.result_file, 'a', encoding='utf-8') as fw:
        ...
        while i < len(selected_idx):
            ...
            print(example["seq"])
            prompt = re_rc_get_prompt(opts, example, resource, r_types, e_types_dict, prompt_list, prompt_icl_list,
                                      prompt_cot_list)
